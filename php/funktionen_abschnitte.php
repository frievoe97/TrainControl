...
function getGegensignal ($infra_id, $alte_fahrtrichtung, $zugtyp = "pz") {

if ($alte_fahrtrichtung == 0) {
$fahrtrichtung = 1;
} else {
$fahrtrichtung = 0;
}
debugMessage ("Suche nächste Abschnitte mit Signal in Fahrtrichtung ".$fahrtrichtung);
$abschnitte = getNaechsteAbschnitte($infra_id, $fahrtrichtung, array("naechstessignal" => true, "zugtyp" => $zugtyp));

// Der Signalabschnitt ist der letzte im Array
if (is_array($abschnitte)) {
return array_pop($abschnitte);
} else {
debugMessage("Kein Signal in Gegenrichtung gefunden!");
return false;
}
}
...
// Setze den Zugtyp
function setZugtyp ($fahrzeug_id, $zugtyp) {
if (!isset($fahrzeug_id) || !isset($zugtyp) || !in_array($zugtyp,array("pz","gz","ra"))) { return false; }

$DB = new DB_MySQL();
$DB->query("UPDATE `".DB_TABLE_FAHRZEUGE."` SET `zugtyp` = '".$zugtyp."' WHERE `id` = '".$fahrzeug_id."'");
unset($DB);
...
// ---------------------------
// Ermittelt den Zugtyp für die Fahrzeugsteuerung
function getZugtyp ($zuggattung_id) {

$DB = new DB_MySQL();
$zugtyp = $DB->select("SELECT `fzs` `".DB_TABLE_ZUEGE_ZUGGATTUNGEN."` WHERE `id` = '".$zuggattung_id."' ");
unset($DB);
